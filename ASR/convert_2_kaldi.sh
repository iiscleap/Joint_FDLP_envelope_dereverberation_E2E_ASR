#!/bin/bash
. ./path.sh
fbankdir="C2LSTM_BEGAN_mse_01gan"
name="arFeat"
nj=5;
while read x
do
...
find $htkfeatdir -name *.fea | awk '{ file=$1; label=$1; gsub(/.*\//,"",label); gsub(/\..*/,"",label); print label" "file; }' | sort > ${htkfeatdir}.scp
  ...
  for n in `seq 1 $nj`; do
    cat $featdir/logmel_${name}.${n}.scp | sed -e "s/_${chn} / \/data2\/ANURENJAN\/VOICES_E2E\/ASR\/FDLP_ENV_CLN_ASR_VOICES_ONLY\/$folder_name\//g" || exit 1;
  done > $data/feats.scp
  rm ${htkfeatdir}.*.scp
  ...
  steps/compute_cmvn_stats.sh \
      /data2/multiChannel/ANURENJAN/VOICES_E2E/ASR/FDLP_ENV_CLN_ASR_VOICES_ONLY/$data exp/make_${fbankdir}/${x} $featdir || exit 1;
#done
