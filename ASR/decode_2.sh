...
# exp tag
tag="TRAIN_MODEL_WITH_WSJ" # tag for managing experiments.
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
 ...
 #cat data-fbank/${train_set}/text| awk -F ' ' '{print $2}' | tr " " "\n" | sort | uniq | grep "<" > ${nlsyms}
 #cat ${nlsyms}
...
if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
    ...
    dump.sh --cmd "$train_cmd" --nj 32 --do_delta ${do_delta} data-fbank/${train_set}/feats.scp data-fbank/${train_set}/cmvn.ark exp/dump_feats/train ${feat_tr_dir}
    dump.sh --cmd "$train_cmd" --nj 4 --do_delta ${do_delta} data-fbank/${train_dev}/feats.scp data-fbank/${train_set}/cmvn.ark exp/dump_feats/dev ${feat_dt_dir}
    dump.sh --cmd "$train_cmd" --nj 4 --do_delta ${do_delta} data-fbank/${eval_set}/feats.scp data-fbank/${train_set}/cmvn.ark exp/dump_feats/eval ${feat_eval_dir}
fi
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
    ...
    data2json.sh --feat ${feat_tr_dir}/feats.scp --nlsyms ${nlsyms} data-fbank/${train_set} ${dict} > ${feat_tr_dir}/data.json
    data2json.sh --feat ${feat_dt_dir}/feats.scp --nlsyms ${nlsyms} data-fbank/${train_dev} ${dict} > ${feat_dt_dir}/data.json
    data2json.sh --feat ${feat_eval_dir}/feats.scp --nlsyms ${nlsyms} data-fbank/${eval_set} ${dict} > ${feat_eval_dir}/data.json
fi
...
# THIS SCRIPT AFTER RUNNING THE DECODING SCRIPT IS USED TO GIVE THE SCORE

#recog_set="Evaluation_Data_wpe"
recog_set="Development_Data_wpe"
