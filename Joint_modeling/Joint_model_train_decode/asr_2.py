...
"""
Why this sys.path.append command ?
See, espnet is installed in state/partition folder but all the codes 
where you have done your changes are present in this path, therefore it is necessary
to tell the system that you have to look in this particular path for all the codes that you will 
going to call otherwise it will call codes from state partition (which are unedited) and will through error
"""
sys.path.append('/home/rohitk/Workspace/E2E/espnet/espnet')
...
from asr.pytorch_backend.asr_init import freeze_modules
from asr.pytorch_backend.asr_init import load_trained_model
from asr.pytorch_backend.asr_init import load_trained_modules
...
#bp()
sys.path.append('/home/rohitk/Workspace/E2E/espnet/espnet')
# this is where you specify where you enhacement net is present
from nets.pytorch_backend_2.Net_CNN_2LSTM_padding_64filters_last2_batch_one_no_cmvn_30_03_2020_MSE10 import Net
#from nets.pytorch_backend_2.fdlp_env_comp_100hz_factor import fdlp_env_comp_100hz_factor_40
from nets.pytorch_backend_2.e2e_asr import pad_list
from nets.pytorch_backend_2.read_HTK import HTKFeat_read, HTKFeat_write
import nets.pytorch_backend_2.lm.default as lm_pytorch
from nets.pytorch_backend_2.streaming.segment import SegmentStreamingE2E
from nets.pytorch_backend_2.streaming.window import WindowStreamingE2E
...
def open_htk(f, mode=None, veclen=36):
...
def integrate(y, inp):
    ...
    fnum = np.floor((inp.shape[2]-flen)/fhop)+1
    send = (fnum-1)*fhop + flen
    factor = 1
    trim = np.floor(send/factor)
    ...
    sr = 400
    flen = int(0.025*sr)
    wind = np.expand_dims(np.repeat(np.expand_dims(signal.hamming(flen), axis=1), 36, axis=1), axis=1)
    ...
    # bp()
    abcd = conv_integrate
    # This below section take context into consideration and return that
    # particular thing, in e2e we may dont need this context and therefore just we will pass abcd
...
def integrate_eval(y, inp):
    ...
    wind = wind.transpose(2, 1, 0)
    # changed from .cuda() to .cpu() for decoding by anurenjan
    filters = torch.from_numpy(wind).float()
    conv_integrate = nn.functional.conv1d(
        y, weight=filters, stride=4, groups=36)
    conv_integrate = conv_integrate**(0.10)
    conv_integrate = conv_integrate.transpose(2, 1)
...
def file_level_cmvn(x):
    random_m = torch.randn_like(x)*0.0001
    temp = x + random_m
    temp_m = (torch.mean(temp,axis=2)).reshape([x.shape[0],x.shape[1],1])
    temp_v = torch.std(temp,axis=2).reshape([x.shape[0],x.shape[1],1])
    x = (temp - temp_m)/(temp_v)
    del random_m, temp, temp_m, temp_v
    return x

def apply_global_cmvn_torch(x):
    
    """
    THis code do the global cmvn on the data
    usage
    y = apply_global_cmvn_torch(x)    x will be some matrix 

    Point to note is that whatever training data you have used to train your e2e model(seperately)
    during that time you must have calculated the mean and variance of the data, you have to use that mean and variance here also
    to normalise the data 
    """
    gmean = np.load('/home/rohitk/Workspace/E2E/espnet/egs/reverb/asr5/data-fdlp_test/tr_simu_8ch_si284/mean.npy')
    gstd = np.load('/home/rohitk/Workspace/E2E/espnet/egs/reverb/asr5/data-fdlp_test/tr_simu_8ch_si284/std.npy')

    #gmean = np.load('/home/rohitk/Workspace/E2E/espnet/egs/reverb/asr5/data-fdlp_C2LSTM_best_model_2sec_EXP_PHASE4_pretrain_mse8_began_2_TEST/tr_simu_8ch_si284/mean.npy')
    #gstd = np.load('/home/rohitk/Workspace/E2E/espnet/egs/reverb/asr5/data-fdlp_C2LSTM_best_model_2sec_EXP_PHASE4_pretrain_mse8_began_2_TEST/tr_simu_8ch_si284/std.npy')
    ...
    count = gmean[-1]
    mean = gmean[0:-1]/count
    var = gstd[0:-1] / count - mean * mean
    std_floor=1.0e-20
    std = np.maximum(np.sqrt(var), std_floor)
    bias = -mean
    scale = 1/std

    bias_t = torch.from_numpy(bias).unsqueeze(0).unsqueeze(0)
    scale_t = torch.from_numpy(scale).unsqueeze(0).unsqueeze(0)

    batch,time,feats = x.shape
    bias_t = bias_t.repeat(batch,1,1)
    scale_t = scale_t.repeat(batch,1,1)
    x = (x + bias_t.cuda())*scale_t.cuda() 
    
    return x

def apply_global_cmvn_eval(x):
    #bp()
    gmean = np.load('/home/rohitk/Workspace/E2E/espnet/egs/reverb/asr5/data-fdlp_test/tr_simu_8ch_si284/mean.npy')
    gstd = np.load('/home/rohitk/Workspace/E2E/espnet/egs/reverb/asr5/data-fdlp_test/tr_simu_8ch_si284/std.npy')
...
class CustomEvaluator(BaseEvaluator):
    ...
    def __init__(self, model, net1,output_dir, iterator, target, device, ngpu=None):
    ...
    def evaluate(self):
        ...
        with torch.no_grad():
            for batch in it:
                ...
                x_noisy = x[0][:,:,:36]
                #x_noisy = file_level_cmvn(x_noisy)
                x_clean = x[0][:,:,36:72]
                #x_clean = file_level_cmvn(x_clean)
                

                if x_noisy.shape[1] > 800:
                    x_noisy_list = []
                    #x_est_list = []
                    mse_loss = 0
                    count_r = x_noisy.shape[1]//800

                    for i in range(x_noisy.shape[1]//800):
                        if (i+1) == count_r:
                            temp1 = x_noisy[:,i*800:,:]
                            temp2 = x_clean[:,i*800:,:]
                            target = temp2 - temp1
                            output = self.model_2(temp1)
                            criterion = nn.MSELoss()
                            mse_loss += criterion(output,target)
                            after_intergration = integrate(output.permute(0,2,1),temp1.unsqueeze(dim=1))
                            after_intergration = torch.squeeze(after_intergration)
                            x_noisy_list.append(after_intergration)
                        else:
                            temp1 = x_noisy[:,i*800:(i+1)*800,:]
                            temp2 = x_clean[:,i*800:(i+1)*800,:]
                            target = temp2 - temp1
                            output = self.model_2(temp1)
                            criterion = nn.MSELoss()
                            mse_loss += criterion(output,target)
                            ...
                            after_intergration = torch.squeeze(after_intergration)
                            x_noisy_list.append(after_intergration)
                            #x_noisy_list.append(temp1)
                            #x_est_list.append(output)
                    
                    x_noisy_list = torch.cat(x_noisy_list,dim=1)
                    x_noisy_list = apply_global_cmvn_torch(x_noisy_list)
                    x_noisy_list = x_noisy_list.float()
                    #x_noisy = torch.cat(x_noisy_list,dim=1)
                    #x_est = torch.cat(x_est_list,dim=1)
                    #after_intergration = integrate(x_est.permute(0,2,1),x_noisy.permute(0,2,1))
                    #after_intergration = torch.squeeze(after_intergration)
                    #bp()
                    size_m = (torch.ones_like(x[1])*int(x_noisy_list.shape[1]))
                    x_e2e = (x_noisy_list, size_m, x[2])
                    ...
                    criterion = nn.MSELoss()
                    mse_loss = criterion(output,target)
                    after_intergration = integrate(output.permute(0,2,1),x_noisy.unsqueeze(dim=1))
                    after_intergration = torch.squeeze(after_intergration)
                    after_intergration = apply_global_cmvn_torch(after_intergration)
                    after_intergration = after_intergration.float()
                    size_m = (torch.ones_like(x[1])*int(after_intergration.shape[1]))
                    x_e2e = (after_intergration, size_m, x[2])

                
                if float(self.eval_check_error) > mse_loss.item():
                    self.eval_check_error = mse_loss.item()
                    path_to_save_after_train = self.outdir+ 'enhace_model.best'
                    if hasattr(self.model_2, "module"):
                        torch.save(self.model_2.module.state_dict(), path_to_save_after_train)
                    else:
                        torch.save(self.model_2.state_dict(), path_to_save_after_train)
                ...
                with reporter_module.report_scope(observation):
                    ...
                    if self.ngpu == 0:
                        #0.7*self.model(*x_e2e)+0.3*mse_loss.item()
                        self.model(*x)
                    else:
                        # apex does not support torch.nn.DataParallel
                        data_parallel(self.model, x_e2e, range(self.ngpu))
...
class CustomUpdater(StandardUpdater):
    ...
    def update_core(self):
        ...
        # this is the optimizer for the espnet
        optimizer = self.get_optimizer("main")
        # it will tell what epoch you are
        epoch = train_iter.epoch
        # this is enhacement optimizer
        ...
        training data
        what x is ?
        x is nothing but a tuple which conatin three things
        1. x[0] > is the training data, which is nothing but some feature*time matrix
        2. x[1] > is a tumple/array which will tell what is size of your training data, that is if your training data is 100*36 than x[1] will be [100,36] 
        3. x[2] > is nothing but label for your training data
        """
        x = _recursive_to(batch, self.device)
        #bp()
        ...
        #x_e2e = (x[0][:,:,0:36],x[1],x[2])

        # ENHACEMENT PART
        #x_noisy = x[0][:,0:800,:36]
        #x_clean = x[0][:,0:800,36:72]
        #bp()

        """
        for training the joint model, we have passed the matrix as time*(2*feature size) that is time*feature size will be noisy array and rest will clean array
        so here
        initial starting 36 features correspond to noisy one
        x_noisy = x[0][:,:,:36]

        and last 36 correspond to clean one
        x_clean = x[0][:,:,36:72]

        """
        x_noisy = x[0][:,:,:36] 
        # THis fucntion will do file level cmvn of the data
        #x_noisy = file_level_cmvn(x_noisy)
        x_clean = x[0][:,:,36:72] 
        #x_clean = file_level_cmvn(x_clean)

        """
        now the matrix is broken down into 800 chunks to perform the enhacement and integration
        """
        ...
        if x_noisy.shape[1] > 800:
            ...
            for i in range(x_noisy.shape[1]//800):
                if (i+1) == count_r:
                    ...
                    #x_noisy_list.appensd(temp1)
                    #x_clean_list.append(temp2)
                    # x_noisy = torch.stack(x_noisy_list)
                    # x_noisy = torch.reshape(x_noisy,[x_noisy.shape[0]*x_noisy.shape[1],x_noisy.shape[2],x_noisy.shape[3]])
                    # x_clean = torch.stack(x_clean_list)
                    # x_clean = torch.reshape(x_clean,[x_clean.shape[0]*x_clean.shape[1],x_clean.shape[2],x_clean.shape[3]])
                    target = temp2 - temp1
                    #output = self.model_2(x_noisy)
                    output = self.model_2(temp1)
                    criterion = nn.MSELoss() 
                    mse_loss += criterion(output,target)
                    after_intergration = integrate(output.permute(0,2,1),temp1.unsqueeze(dim=1)) 
            ...
            """
            after you have integrated them you were appending those time*198 matrix into list right!
            so here you just concatenate them to and get the same matrix size as your were doing seperately
            """
            x_noisy_list = torch.cat(x_noisy_list,dim=1)
            ...
            # Now this matrix will go in the end to end asr
            size_m = (torch.ones_like(x[1])*int(x_noisy_list.shape[1]))
            x_e2e = (x_noisy_list, size_m, x[2])
            #x_e2e = (x_noisy_list, x[1], x[2])
            #temp = integrate(output.permute(0,2,1),x_noisy.permute(0,2,1))
        else:
            ...
            #after_intergration = integrate(output.permute(0,2,1),x_noisy.permute(0,2,1))
            #after_intergration = (torch.squeeze(after_intergration))
            #size_m = (torch.ones_like(x[1])*int(after_intergration.shape[1]))
            #x_e2e = (after_intergration, size_m, x[2])
        ...
        if self.ngpu == 0:
            ...
            loss_e2e = self.model(*x_e2e).mean() / self.accum_grad
            loss = loss_e2e + 0.4*mse_loss
        else:
            # apex does not support torch.nn.DataParallel
            """
            pass now the e2e stuff thorugh the model
            """
            loss_e2e = (
                data_parallel(self.model, x_e2e, range(self.ngpu)).mean() / self.accum_grad
            )
            # no mse loss
            #EXP 1 THIS LOSS
            # just using the e2e loss
            loss = loss_e2e
            
            #EXP 2 

            #loss =  0.7*loss_e2e + 0.3*mse_loss
            #loss = loss_e2e + 0.4*mse_loss
    ...
    def update(self):
        """
        Training loop, this updater will call the forward loop again and again to train the model
        """
        self.update_core()
...
def train(args):
    ...
    # THis line is being added to divide the dimension

    idim_list = [int(idim_list[0]/2)]
    ...
    if (args.enc_init is not None or args.dec_init is not None) and args.num_encs == 1:
        ...
        sys.path.append('/home/rohitk/Workspace/E2E/espnet/espnet')
        from utils.dynamic_import_2 import dynamic_import
    ...
    """
    this is where you put your enhacement model
    """
    # This section is edited by Rohit K, in this we will take the enhacement model and load the 
    # the already trained model into the asr pipeline
    net1 = Net()
    net1 = net1.cuda()
    #path_of_model = '/data2/multiChannel/ROHITK/Workspace/FDLP_Feats_Extraction/egs/REVERB/Length_trial_train/exp/torch_ENV_CLN_4CNN_3LSTM_2SEC_EXP_baseline/Exp_1_Adding_Transfomer_2_sec.model'
    # this lower path is for began one
    #path_of_model = '/data2/multiChannel/ROHITK/Workspace/FDLP_Feats_Extraction/egs/REVERB/Length_trial_train/exp/EXP_PHASE4_pretrain_mse8_began_2/best_model_enh.model'
    path_of_model = '/data2/multiChannel/ROHITK/Workspace/FDLP_Feats_Extraction/egs/REVERB/Length_trial_train/exp/EXP_PHASE3_mse_1began_bse32_pretrain/best_model_enh.model'
    net1.load_state_dict(torch.load(path_of_model))
    optimizer1 = torch.optim.Adam(net1.parameters(), lr=0.0001, amsgrad=True)
    ...
    if args.save_interval_iters > 0:
        trainer.extend(
            CustomEvaluator(model, net1,args.outdir, {"main": valid_iter}, reporter, device, args.ngpu),
...
def recog(args):
    ...
    #THis section to load the enhacement model
    #bp()
    kk = args.model.split('/')[:-1]
    kk.append('enhace_model.best')
    mpath = '/'.join(kk)
    #path_of_model = '/home/rohitk/Workspace/E2E/espnet/egs/reverb/JT_ASR/exp/tr_simu_8ch_si284_pytorch_EXP0_ENV_CLEAN_BASELINE_MODEL_ASR_TRANS/resultsenhace_model.best'
    #path_of_model = '/data2/multiChannel/ROHITK/Workspace/FDLP_Feats_Extraction/egs/REVERB/Length_trial_train/exp/torch_ENV_CLN_4CNN_3LSTM_2SEC_EXP_baseline/Exp_1_Adding_Transfomer_2_sec.model'
    #path_of_model = '/home/rohitk/Workspace/E2E/espnet/egs/reverb/JT_ASR/exp/tr_simu_8ch_si284_pytorch_EXP0_ENV_CLEAN_BASELINE_MODEL_ASR_TRANS_WITH_WEIGHTS/resultsenhace_model.best'
    path_of_model = '/home/rohitk/Workspace/E2E/espnet/egs/reverb/JT_ASR/exp/tr_simu_8ch_si284_pytorch_EXP_WITH_BEGAN_WEIGHT/resultsenhace_model.best'
    #net1.load_state_dict(torch.load(path_of_model))
    net1 = Net()
    #net1.load_state_dict(torch.load(mpath))
    net1.load_state_dict(torch.load(path_of_model))
    #torch_load(mpath,net1)
    net1.eval()
    ...
    if args.batchsize == 0:
        with torch.no_grad():
            ...
            for idx, name in enumerate(js.keys(), 1):
                ...
                #bp()
                #near_fea = '/data2/multiChannel/ANURENJAN/REVERB/For_ROHIT/E2E_TR_DATA/REVERB_tr_cut/SimData_tr_for_1ch_A/newfea/011c0201.fea'
                #data_reverb_near = open_htk(str(near_fea), 'rb')
                #data_reverb_near = data_reverb_near.getall()
                #data_reverb_near = (data_reverb_near - data_reverb_near.mean(axis=0)) / data_reverb_near.std(axis=0)
                #feat = torch.from_numpy(data_reverb_near).unsqueeze(dim=0)
                
                feat = torch.from_numpy(feat).unsqueeze(dim=0)
                #feat = file_level_cmvn(torch.from_numpy(feat[:,0:36]).unsqueeze(dim=0))

                if feat.shape[1] > 800:
                    x_noisy_list = []
                    x_est_list = []
                    #count = 0
                    count = feat.shape[1]//800
                    for i in range(feat.shape[1]//800):
                        if (i+1) == count:
                            temp1 = feat[:,i*800:,:]
                            output = net1(temp1)
                            after_intergration = integrate_eval(output.permute(0,2,1),temp1.unsqueeze(dim=1))
                            after_intergration = torch.squeeze(after_intergration)
                            x_noisy_list.append(after_intergration)
                        else:
                            temp1 = feat[:,i*800:(i+1)*800,:]
                            output = net1(temp1)
                            after_intergration = integrate_eval(output.permute(0,2,1),temp1.unsqueeze(dim=1)) 
                            after_intergration = torch.squeeze(after_intergration)
                            x_noisy_list.append(after_intergration)
                    ...
                    #bp()
                    # Algo: repeating the last frame and make number of frames 800
                    #temp1 = feat[:,count*800:,:]

                    # Algo: passing the left out frames along with some of the past frames such that number of frames is 800
                    # selecting the last 800 col
                    """temp1 = feat[:,-800:,:]
                    output = net1(temp1)
                    after_intergration_left = integrate_eval(output.permute(0,2,1),temp1.unsqueeze(dim=1)) 
                    after_intergration_left = torch.squeeze(after_intergration_left)

                    index = []
                    i=0
                    for x in after_intergration:
                        if (torch.abs(after_intergration_left[0,:]-x) < 1e-2).all():
                            index.append(i)
                        i += 1
                    ...
                    x_noisy_list = torch.cat(x_noisy_list,dim=0)
                    #x_est = torch.cat(x_est_list,dim=1)
                    #after_intergration = integrate_eval(x_est.permute(0,2,1),x_noisy_list.permute(0,2,1))
                    #after_intergration = torch.squeeze(after_intergration)
                    
                    feat = x_noisy_list.cpu().detach().numpy()
                    feat = apply_global_cmvn_eval(feat)
                    feat = feat.astype(np.float32)
                else:
                    temp1 = feat
                    output = net1(temp1)
                    after_intergration = integrate_eval(output.permute(0,2,1),temp1.permute(0,2,1))
                    after_intergration = torch.squeeze(after_intergration)
                    feat = after_intergration.cpu().detach().numpy()
