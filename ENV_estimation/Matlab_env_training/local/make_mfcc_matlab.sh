...
hcopybin=/home/sriram/speech/Aurora4/kaldiSetup/scripts_matlab_logmel/local/expt/logmel/extract_feats.sh
...
featdir=$plpdir/$name.htk; mkdir -p $featdir || exit 1;
...
if [ $stage -le 0 ]; then
  ...
  if [ $vtln_map ]; then
    ...
    $cmd -tc $nj JOB=1:$N $logdir/hcopy/hcopy-vtln.JOB.log \
      ...
      echo "HPARM: WARPLCUTOFF = 3400" '>>' \$conf '&&' \
      echo "HPARM: WARPUCUTOFF = 3400" '>>' \$conf '&&' \
      $hcopybin -A -D -T 5 -C \$conf \$wav $featdir/\$name.fea
  else
    echo "Computing Features (HTK)"
    $cmd -tc $nj JOB=1:$N $logdir/hcopy/hcopy-novtln.JOB.log \
      eval "wav=\$(cat $scp | head -n JOB | tail -n 1 | awk '{ print \$(NF-4) }');" '&&' \
      eval "name=\$(cat $scp | head -n JOB | tail -n 1 | awk '{ print \$1 }');" '&&' \
      $hcopybin \$wav $featdir/\$name.fea
  fi 
fi
...
if [ $stage -le 1 ]; then
  ...
  featdir=$plpdir/$name.kaldi; mkdir -p $featdir || exit 1;
  htkfeatdir=$plpdir/$name.htk
