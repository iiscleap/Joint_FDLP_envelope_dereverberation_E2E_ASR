...
#          print ('[%d,%5d] loss_tr: %.3f' % (epoch+1,i+1,running_loss_tr/trGen.numSteps))
          print ('TRAIN ==> [%d,%5d] LOSS: %.3f '%(epoch+1,i+1,running_loss_tr/2000))
          print ('TRAIN ==> [%d,%5d] LOSS: %.3f '%(epoch+1,i+1,running_gan_loss_tr/2000))
          running_loss_tr =0.0
          running_gan_loss_tr = 0.0
...
#corss validation step   
        ...
        for j, data in enumerate(cvGen,0):
         ...
         if j % (200) == ((200) - 1):
           ...
           self.gan_losscv_current=running_gan_loss_cv/(200)
           
           print ('VALID ==> [%d,%5d] LOSS_CV: %.3f '%(epoch+1,j+1,running_loss_cv/(200)))
           print ('GAN_VALID ==> [%d,%5d] GAN_LOSS_CV: %.3f '%(epoch+1,j+1,running_gan_loss_cv/(200)))
           running_loss_cv =0.0
           running_gan_loss_cv =0.0
