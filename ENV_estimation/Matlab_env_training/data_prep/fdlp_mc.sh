...
echo "pwdr is ${pwdr}"
$cpython utils/split_scp.py --splits $nj --file ${pwdr}/filelist.scp --prefix filelist --dest ${pwdr}/splits
...
for job in `seq 1 $nj`; do
 ...
 qsub -l hostname=compute-0-[0-4] -e $err -o $out -S /bin/bash $hcopybin $matlab_path ${pwdr}/newfea/split_$job \
