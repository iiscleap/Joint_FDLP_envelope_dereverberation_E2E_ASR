...
refdirbase=`echo "/home/data2/multiChannel/ANURENJAN/VOICES/ENV_estimation/Matlab_env_training/data-Input_training_data/train_clean_100" | sed -e 's/_A//g'` #changed here from reverb
echo $refdirbase
...
find ${htkfeatdir} -name *.fea | awk '{ file=$1; label=$1; gsub(/.*\//,"",label); gsub(/\..*/,"",label); print label" "file; }' | sort > ${htkfeatdir}.scp
 ...
 featdir=${dirbase}.kaldi; mkdir -p $featdir || exit 1;
  ...
  for n in `seq 1 $nj`; do
    cat $featdir/logmel_${name}.${n}.scp | sed -e "s/_ / \/home\/data2\/ANURENJAN\/VOICES\/ENV_estimation\/Matlab_env_training\/$folder_name\//g" || exit 1;
  done > $data/feats.scp
  rm ${htkfeatdir}.*.scp
  utils/fix_data_dir.sh data-Input_training_data/train_clean_100
  steps/compute_cmvn_stats.sh \
      $cmvn_dir/$data exp/make_${fbankdir}/${x} $featdir || exit 1;


done < data_prep/dirList_cln_train 
