...
refdirbase=`echo "/home/data2/multiChannel/ANURENJAN/VOICES/ENV_estimation/Matlab_env_training/old_data-Input_training_data/$x" | sed -e 's/_A.htk//g'` #changed here from reverb
echo $refdirbase
...
find $htkfeatdir -name *.fea | awk '{ file=$1; label=$1; gsub(/.*\//,"",label); gsub(/\..*/,"",label); print label" "file; }' | sort > ${htkfeatdir}.scp
  ...
  for n in `seq 1 $nj`; do
    cat $featdir/logmel_${name}.${n}.scp | sed -e "s/_${chn} / \/home\/data2\/ANURENJAN\/VOICES\/ENV_estimation\/Matlab_env_training\/$folder_name\//g" || exit 1;
  done > $data/feats.scp
  rm ${htkfeatdir}.*.scp
...
done < data_prep/dirList_training
