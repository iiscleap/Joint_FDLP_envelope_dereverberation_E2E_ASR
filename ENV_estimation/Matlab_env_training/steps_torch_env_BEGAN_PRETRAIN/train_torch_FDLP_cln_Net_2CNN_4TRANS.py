...
import numpy
import sys
from NET.Net_POS_4TRANS import Net
#from NET.Net_CNN_2LSTM_padding_64filters_encoder_transformer import Net
...
#           print ('Finished Crossvalidation')
    #       if self.losscv_current <= self.losscv_previous_1:
           ...
           torch.save(net.state_dict(),exp + '/Exp_1_Adding_Transfomer_2_sec.model')
           print('Saved model')
           break
           #else:
           # break
...
for epoch in range(learning['minEpoch']-1):
 if(epoch >= 1):
         net.load_state_dict(torch.load(exp + '/Exp_1_Adding_Transfomer_2_sec.model'))
 t0 = time.time()
