...
class dataGeneratorCNN_multiAll:
	...
	#bp()
        ...
        self.y = numpy.empty ((0, 1, 198,self.inputFeatDim), dtype=numpy.float32)
        self.batchPointer = 0
...
#        p3 = Popen (yy=None, stdin=p2.stdout, stdout=PIPE) # no deltas in processing
	...
	outputList =[]
	 ...
	 trim=800*f 
	 featListFinal =featMat.reshape(1,self.spliceSize,featMat.shape[0],self.inputFeatDim)
	 ...
	 featMat_tgt = featMat_tgt.reshape(1,self.spliceSize,featMat_tgt.shape[0],self.inputFeatDim)
	 ...
	 #bp()
 	 ...
 	 featMat_tgt_tp=numpy.empty((1,1,198,36))
	 for x in range(f):
		
			temp1            = featListFinal_tp
			temp2            =  featMat_tgt_tp
			featListFinal_tp = numpy.concatenate((temp1,featListFinal[:,:,((x)*800):((x+1)*800),:]),axis=0)
			temp3            =  featMat_tgt[:,:,((x)*800):((x+1)*800),:] - featListFinal[:,:,((x)*800):((x+1)*800),:]   # taking difference between the clean spectra
                                                                                                                                    # reverberant spectra to get the residual as the target  
			temp3            =  numpy.exp(temp3)
			temp3            =  fdlp_env_comp_100hz_factor_40(temp3[0,0,:,:].T,400, 36)
			temp3            =  numpy.expand_dims(numpy.expand_dims(temp3.T,axis= 0),axis=0)
			featMat_tgt_tp   =  numpy.concatenate((temp2,temp3),axis=0)
	 #bp()
	 featListFinal = featListFinal_tp[1:,:,:,:]
	 featMat_tgt   = numpy.log(featMat_tgt_tp[1:,:,:,:])	 
