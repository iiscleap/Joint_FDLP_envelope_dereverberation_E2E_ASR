...
from data_gen.dataGeneratorCNN_multiAll_LSTM_400X36 import dataGeneratorCNN_multiAll
import numpy
import sys
...
#          print ('[%d,%5d] loss_tr: %.3f' % (epoch+1,i+1,running_loss_tr/trGen.numSteps))
          print ('TRAIN ==> [%d,%5d] LOSS: %.3f '%(epoch+1,i+1,running_loss_tr/1500))
          running_loss_tr =0.0
...
#           print ('[%d,%5d] loss_cv: %.3f' % (epoch+1,j+1,running_loss_cv/cvGen.numSteps))
           self.losscv_current=running_loss_cv/(150)
           print ('VALID ==> [%d,%5d] LOSS_CV: %.3f '%(epoch+1,j+1,running_loss_cv/(150)))
           running_loss_cv =0.0
