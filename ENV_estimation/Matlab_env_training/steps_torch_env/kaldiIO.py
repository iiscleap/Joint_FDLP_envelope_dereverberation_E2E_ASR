...
def writeUtterance (uttId, featMat, ark, encoding):
    ...
    m,n = featMat.shape
    ## Write header
    ark.buffer.write (struct.pack('<%ds'%(len(uttId)),uttId.encode(encoding)))
    ark.buffer.write (struct.pack('<cxcccc',' '.encode(encoding),'B'.encode(encoding),
                'F'.encode(encoding),'M'.encode(encoding),' '.encode(encoding)))
    ark.buffer.write (struct.pack('<bi', 4, m))
    ark.buffer.write (struct.pack('<bi', 4, n))
    ## Write feature matrix
    ark.buffer.write (featMat)
