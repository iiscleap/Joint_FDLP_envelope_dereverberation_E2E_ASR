...
class Net (nn.Module):
    def __init__(self):
        ...
        self.lstm  = nn.LSTM(152 , 48 , num_layers=2 , batch_first=True) 
        #self.fc1   = nn.Linear(38*3*8,1024)
        #self.drop3 = nn.Dropout(0.2)
        #self.fc2   = nn.Linear(1024,1024)
        #self.drop4 = nn.Dropout(0.2)
        self.fc3   = nn.Linear(48,800*36)
        self.relu  = nn.ReLU()
        #self.tanh  = nn.Tanh()
    ...
    def forward(self, x):
        ...
        x = self.relu(x[:,:,47])   ############### (B,seq_len, 128) ==>> remove seq_len to get (32,128) try the other way as well i.e.,(B,48)
	x = self.fc3(x)
        return x
