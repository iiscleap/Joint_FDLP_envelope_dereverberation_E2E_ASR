...
class dataGeneratorCNN_multiAll:
	...
	#bp()
        ...
        self.y = numpy.empty ((0, 1, self.frameLen,self.inputFeatDim), dtype=numpy.float32)
        self.batchPointer = 0
...
#        p3 = Popen (yy=None, stdin=p2.stdout, stdout=PIPE) # no deltas in processing
	...
	outputList =[]
	 ...
	 #bp()
 	 ...
 	 featMat_tgt_tp=numpy.empty((1,1,400,36))
	 for x in range(f):
		
			temp1=featListFinal_tp
			temp2=featMat_tgt_tp
			featListFinal_tp=numpy.concatenate((temp1,featListFinal[:,:,((x)*400):((x+1)*400),:]),axis=0)
			featMat_tgt_tp=numpy.concatenate((temp2,featMat_tgt[:,:,((x)*400):((x+1)*400),:]),axis=0)
	 #bp()
