...
#        p3 = Popen (yy=None, stdin=p2.stdout, stdout=PIPE) # no deltas in processing
	...
	outputList =[]
	 ...
	 #bp()
 	 featListFinal_tp=numpy.empty((1,1,200,36))
 	 featMat_tgt_tp=numpy.empty((1,1,200,36))
	 for x in range(f):
		
			temp1=featListFinal_tp
			temp2=featMat_tgt_tp
			featListFinal_tp=numpy.concatenate((temp1,featListFinal[:,:,((x)*200):((x+1)*200),:]),axis=0)
			featMat_tgt_tp=numpy.concatenate((temp2,featMat_tgt[:,:,((x)*200):((x+1)*200),:]),axis=0)
	 #bp()
