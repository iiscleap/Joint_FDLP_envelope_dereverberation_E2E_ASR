function feats=modulationBandpassFilt_features(samples,sr,num_ceps,flag_delta,energy,t_bandWidths,f_bandWidths)
    ...
    specStreams=modulationBandpassFilt(samples, sr, t_bandWidths, f_bandWidths);
    
    feats={};
    dim=size(specStreams);
    for i=1:dim(1)
        for j=1:dim(2)
            feats{i,j}=getCepstra(specStreams{i,j},num_ceps,energy,flag_delta);
...
function feats=getCepstra(postspectrum,num_ceps,energy,flag_delta)
    if energy == 0
        cepstra = spec2cep(postspectrum,num_ceps+1);
        cepstra = cepstra(2:end,:);
    else 
        cepstra = spec2cep(postspectrum,num_ceps);
    end
