...
ptmp1 = fdlpfit_full_sig(x,sr,dB,do_gain_norm, 'mel');
ptmp2 = fdlpfit_full_sig2(x,sr,dB,do_gain_norm, 'mel');

nb = size(ptmp1,1);                                  % Number of Sub-bands            
p1 = cell(nb,1);                                     % Initialize the pole cell array        
p2 = cell(nb,1);                                     % Initialize the pole cell array        
start_band =1;
...
ENV1 = zeros(floor(fdlplen/factor), nb);
ENV2 = zeros(floor(fdlplen/factor), nb);
ENV = zeros(floor(fdlplen/factor), nb);

for J = start_band:(nb)
    ENV1(:,J)  =  fdlpenv(p1(J),floor(fdlplen/factor));
    ENV2(:,J)  =  fdlpenv(p2(J),floor(fdlplen/factor));
    ENV(:,J) = ENV1(:,J)  ./ (ENV2(:,J).^(0.2)) ;
end
