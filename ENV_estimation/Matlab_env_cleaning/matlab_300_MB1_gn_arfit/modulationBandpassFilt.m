function tfStreams = modulationBandpassFilt(samples, sr, t_bandWidths, f_bandWidths)
   
    %pre-emphasis
    samples = samples - mean(samples);
    samples = samples - 0.97* [0 ; samples(1:end-1)];      

    tWindow=100; %since 100 frames are sufficient to capture the frequency response of the filter
    
    %% get the auditory spectrogram (in dB scale)
    %audSpectrum=getAudSpec(samples,sr);
...
%% do modulation bandpass filtering of the spectrum
   ...
   %Downsampling the spectrograms  
    for i=1:length(tfStreams(:))
        s=size(tfStreams{i});    
        aspectrum=zeros(s(1)/4,s(2));
        for j=1:s(2)
            aspectrum(:,j)=decimate(tfStreams{i}(:,j),4); %to produce 32 band output
        end
        tfStreams{i}=aspectrum;
    end
