...
if $fake; then
  ...
  echo "--spk2utt=ark:$data/spk2utt scp:$data/feats.scp ark,scp:$cmvndir/cmvn_$name.ark,$cmvndir/cmvn_$name.scp"
  ! compute-cmvn-stats scp:$data/feats.scp ark,scp:$cmvndir/cmvn_$name.ark,$cmvndir/cmvn_$name.scp \
    2> $logdir/cmvn_$name.log && echo "Error IIIINNN computing CMVN stats. See $logdir/cmvn_$name.log";
fi
echo "$cmvndir/cmvn_$name.scp $data/cmvn.scp"
cp $cmvndir/cmvn_$name.scp $data/cmvn.scp || exit 1;

nc=`cat $data/cmvn.scp | wc -l`
nu=`cat $data/spk2utt | wc -l`
echo "$nc should be = to $nu"
if [ $nc -ne $nu ]; then
