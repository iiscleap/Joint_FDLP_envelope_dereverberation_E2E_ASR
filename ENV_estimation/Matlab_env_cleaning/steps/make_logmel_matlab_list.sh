...
hcopybin=/home/anirudhs/IISC/REVERB/Reverb_fdlp_36bands_300pole_ganinorm0/local/expt/logmel/generate_feats.sh
...
echo "feats directory is $plpdir"
...
name=`basename $data`
echo "plpdri is ===== $plpdir"
dirbase=`dirname $plpdir`
echo "dirbase is ==== $dirbase"
dirbase=$dirbase/fdlp_feats
...
featdir=${plpdir}/${name}.htk; 

mkdir -p $featdir || exit 1;

rm -f $featdir/filelist.scp
echo "featsdir is $featdir"
#if [ $stage -le 0 ]; then
 ...
 echo 'stage'
  ...
  #$hcopybin -A -D -T 5 -C \$conf \$wav $featdir/$name.fea #changed here
 #else
  echo "Computing Features (MATLAB)"
  echo $scp
  echo "data is $data"	
  #if  [ "$data" -eq "data/tr05_orig_clean" ] -o [ "$data" -eq "data/dt05_orig_clean" ] -o [ "$data" -eq "data/et05_orig_clean" ] ; then
  
  
   echo 'hello NF-1'
    for  JOB in `seq 1 $N` ; do  
    
    wav=`cat $scp | head -n $JOB | tail -n 1 | awk '{ print $2 }'`
...
echo "spliting the tasks to create 10 jobs"
python3 utils/split_scp.py --splits 30 --file ${featdir}/filelist.scp --prefix filelist --dest ${featdir}


echo "Generating Features From List"
for job in `seq 1 30`; do
  out=`echo "$logdir/feats_file.$job.out"`
  err=`echo "$logdir/feats_file.$job.err"`
 qsub  -e $err -o $out -S /bin/bash $hcopybin $featdir/split_$job \
                                      $featdir/filelist.$job.scp 
done

sleep 15s
echo "Waiting for Feats to be done ..."
cnter=`ls $featdir/*.fea | wc -l | awk '{print $1}'`
#cnter=`ls Chime3/fdlp/tr05_real_noisy.htk/*.fea | wc -l | awk '{print $1}'`
#fdlp/.htk
while [ "$cnter" -lt $N ]
do
   sleep 15s
...
if [ $stage -le 1 ]; then
  ...
  featdir=$dirbase/${name}.kaldi; mkdir -p $featdir || exit 1; #changed here
  htkfeatdir=$dirbase/${name}.htk #changed here for "fdlp"
  echo $featdir
  echo $htkfeatdir
  echo $name
