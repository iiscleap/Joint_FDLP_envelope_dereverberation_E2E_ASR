...
find $htkfeatdir -name *.fea | awk '{ file=$1; label=$1; gsub(/.*\//,"",label); gsub(/\..*/,"",label); print label" "file; }' | sort > ${htkfeatdir}.scp
  ...
  for n in `seq 1 $nj`; do
    cat $featdir/logmel_${name}.${n}.scp | sed -e "s/_${chn} / \/data2\/ANURENJAN\/VOICES\/ENV_estimation\/Matlab_env_cleaning\/$folder_name\//g" || exit 1;
  done > $data/feats.scp
  rm ${htkfeatdir}.*.scp
  ...
  steps/compute_cmvn_stats.sh \
      /data2/multiChannel/ANURENJAN/VOICES/ENV_estimation/Matlab_env_cleaning/$data exp/make_${fbankdir}/${x} $featdir || exit 1;
#done
