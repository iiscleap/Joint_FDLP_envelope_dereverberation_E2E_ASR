...
_E = 0o100    #0000100 ## has energy
_N = 0o200    #0000200 ## absolute energy supressed
_D = 0o400         #0000400 ## has delta coefficients
_A = 0o1000         #0001000 ## has acceleration (delta-delta) coefficients
_C = 0o2000    #0002000 ## is compressed
_Z = 0o4000      #0004000 ## has zero mean static coefficients
_K = 0o10000    #0010000 ## has CRC checksum
_O = 0o20000   #0020000 ## has 0th cepstral coefficient
_V = 0o40000   #0040000 ## has VQ data
_T = 0o100000  #0100000 ## has third differential coefficients

def open_i(f, mode=None, veclen=13):
...
class HTKFeat_read(object):
    ...
    def __init__(self, filename=None):
        ...
        if (filename != None):
            self.open_w(filename)

    def __iter__(self):
        self.fh.seek(12,0)
        return self

    def open_w(self, filename):
...
bp()
data_in1 = open_i('t6c020d.fea', 'rb') 
bp()
data_original = data_in1.getall()
data_original = data_original.astype("float32")
write_htk( 't6c020d_trial.fea' , data_original , 100000 , 8267 )
data_in2 = open_i('t6c020d_trial.fea', 'rb') 
bp()
data_original2 = data_in2.getall()
bp()
