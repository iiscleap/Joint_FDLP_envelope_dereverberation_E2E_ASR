...
def forward_pass(data_original, exp, model_name, in_channel=1, inputFeatDim=36):
  ...
  outputs = outputs.reshape(-1,1,198,36)
  cepstra_in_exp = numpy.exp(cepstra_in)
  #bp()
  if f == 0:
      data = numpy.transpose(cepstra_in_exp[f,0,:,:])
      Intout = fdlp_env_comp_100hz_factor_40(data[:,0:len_in],400, 36)
      cepstra = Intout
  else :     
      for i in range(f):
       if i == f-1 :
          data = numpy.transpose(numpy.concatenate((cepstra_in_exp[i,0,:,:],cepstra_in_exp[f,0,0:len_in-trim,:])))
          Intout = fdlp_env_comp_100hz_factor_40(data,400, 36)
       else : 
          data = numpy.transpose(cepstra_in_exp[i,0,:,:])
          Intout = fdlp_env_comp_100hz_factor_40(data,400, 36)
       if i == 0:
          cepstra = Intout
       else :
  ...
  cepstra_in_int = numpy.log(cepstra) 
  len_in_2 = cepstra_in_int.shape[1]
  trim_2 =  198*f    
  if f == 0:
      data = numpy.transpose(outputs[f,0,:,:])
      cepstra = data[:,0:len_in_2]
  else :     
      for i in range(f):
       if i == f-1 :
          data = numpy.transpose(numpy.concatenate((outputs[i,0,:,:],outputs[f,0,0:len_in_2-trim_2,:])))

       else : 
          data = numpy.transpose(outputs[i,0,:,:])
          
       if i == 0:
          cepstra = data
       else :
          cepstra = numpy.concatenate((cepstra,data),axis=1)  
  
  outputs = cepstra + cepstra_in_int
  return outputs
