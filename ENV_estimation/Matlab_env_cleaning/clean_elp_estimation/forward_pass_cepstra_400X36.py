...
from NET.Net_CNN_2LSTM_padding_64filters_last2_400x36 import Net
from pdb import set_trace as bp  #################added break point accessor####################
from fdlp_env_comp_100hz_factor_40_400X36 import fdlp_env_comp_100hz_factor_40
...
def forward_pass(data_original, exp, model_name, in_channel=1, inputFeatDim=36):
  ...
  f=data_feat_unpack.shape[0]//400
  trim=400*f 
  #bp()
  untrim = numpy.zeros((400-data_feat_unpack.shape[0]+trim,36)) # (trim - len_in) == add_sample in matlab
  ...
  featListFinal_tp=numpy.empty((1,1,400,36))
   	
  for x in range(f+1):
     temp1=featListFinal_tp
     featListFinal_tp=numpy.concatenate((temp1,featListFirst[:,:,((x)*400):((x+1)*400),:]),axis=0)
  ...
  #bp()
  outputs = outputs.reshape(-1,1,400,36)
  outputs = outputs + cepstra_in
  outputs_cat = numpy.empty([400,36])
  #bp()
  for x in range(outputs.shape[0]):
     outputs_cat = numpy.concatenate((outputs_cat,outputs[x,0,:,:]),axis=0) 
  #bp()  
  outputs_cat = outputs_cat[400:data_original.shape[0]+400,:]
  #bp()
  data_feat_unpack = outputs_cat
  len_in = data_feat_unpack.shape[0]
